  ...
  async execute(interaction, client) {
    ...
    if (!interaction.isButton() && !interaction.isModalSubmit()) {
      ...
      if (interaction.customId.includes("ticketModal")) {
        ...
        const confirmDeny = await interaction.reply({ embeds: [confirmationEmbed], components: [confirmationButtons], fetchReply: true })

        const collectorFilter = i => i.user.id === interaction.user.id;

        try {
          const confirmation = await confirmDeny.awaitMessageComponent({ filter: collectorFilter, time: 180_000 })
